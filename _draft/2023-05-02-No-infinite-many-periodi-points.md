...
Consider an elliptic curve $E$ defined over $\mathbb{Q}$ and let $[2]$ denote the multiplication-by-$2$ endomorphism of $E$. Let $E(K)$ be the $K$-rational point of $E$. We define $f=[2]_x$ as the rational function obtained by projecting $[2]$ onto the $x$-coordinate. Let $O$ be the identity on $E$, and let $Q = (Q_x, Q_y)$ be a non-torsion point of $E(\mathbb{Q})$. If $Q_1$ and $Q_2$ are distinct and satisfy $[2^n](Q_1) = [2^n](Q_2) = Q$ for some positive integer $n$, then $[2^n](Q_1-Q_2) = O$. It implies that $\mathbb{Q}_f(Q_x)$ contains all preimages of fixed points, and so are preperiodic point of $f$. 

On the other hand, let us investigate the periodic point of $f$. Suppose $P=(P_x,P_y)\in E$ satisfies that $P_x$ is a periodic point of $f$. Then, $P$ is a preperiodic point of $[2]$. Note that only $Q$ and $-Q$ has the same $x$-coordinate, so therefore we have
$$ [2^n]P = P\text{ and }[2^n]P = -P.$$
It implies that $P$ is either $2^n-1$ or $2^n+1$ torsion points. However, there are only finitely many $2^n-1$ or $2^n+1$ torsion points in $E(\mathbb{Q})$, and doesn't have extra $2^n-1$ and $2^n+1$ torsion points in $E(\mathbb{Q}_f(Q_x))$. Hence, we once again conclude that $\mathbb{Q}_f(Q_x)\cap\text{Per}(f)=\text{Per}(f,\mathbb{Q})$.
